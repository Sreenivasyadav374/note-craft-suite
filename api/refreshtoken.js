...
export default async function handler(req, res) {
  ...
  const tokenId = parsedUrl.searchParams.get("id") || req.url.split("/").pop();
  ...
  try {
    ...
    switch (req.method) {
      case 'GET':
        if (tokenId) {
          const token = await collection.findOne({ id: tokenId });
      ...
      case 'POST':
        const newToken = await parseBody(req);
      ...
      case 'DELETE':
        await collection.deleteOne({ id: tokenId });
